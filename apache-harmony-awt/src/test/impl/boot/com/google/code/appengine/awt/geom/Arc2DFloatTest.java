...
public class Arc2DFloatTest extends GeomTestCase {
    ...
    public Arc2DFloatTest(String name) {
        super(name);
    }
    ...
    public void testCreate1() {
        assertEquals(new Arc2D.Float(0, 0, 0, 0, 0, 0, Arc2D.OPEN), new Arc2D.Float());
    }
    ...
    public void testIsEmpty() {
        ...
        assertFalse(new Arc2D.Float(0, 0, 1, 1, 2, 3, Arc2D.CHORD).isEmpty());
        assertTrue(new Arc2D.Float(0, 0, 0, 1, 2, 3, Arc2D.CHORD).isEmpty());
        assertTrue(new Arc2D.Float(0, 0, 1, 0, 2, 3, Arc2D.CHORD).isEmpty());
        assertTrue(new Arc2D.Float(0, 0, 0, 0, 2, 3, Arc2D.CHORD).isEmpty());
        assertTrue(new Arc2D.Float(0, 0, -1, 0, 2, 3, Arc2D.CHORD).isEmpty());
        assertTrue(new Arc2D.Float(0, 0, 0, -1, 2, 3, Arc2D.CHORD).isEmpty());
        assertTrue(new Arc2D.Float(0, 0, -1, -1, 2, 3, Arc2D.CHORD).isEmpty());
    }

    public void testSetArc() {
        a.setArc(7, 8, 9, 10, 11, 12, Arc2D.CHORD);
        assertEquals(new Arc2D.Float(7, 8, 9, 10, 11, 12, Arc2D.CHORD), a);
    }
    ...
    public void testMakeBounds() {
        assertEquals(new Rectangle2D.Float(1, 2, 3, 4), a.makeBounds(1, 2, 3, 4));
    }

    public static void main(String[] args) {
        junit.textui.TestRunner.run(Arc2DFloatTest.class);
    }
